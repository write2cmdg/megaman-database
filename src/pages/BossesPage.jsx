        ...
        <div className=' w-full text-blue-200'>
          {
            data.map((d, ind) => (
              ...
              <div className='w-full flex flex-col items-center justify-between h-28' key={ind}>

                <div className='w-3/4 flex flex-row justify-around items-center'>
                  
                  <Link className="mx-auto basis-1/2 sm:text-3xl text-lg capitalize hover:text-blue-200/50" to={`/oneBoss/${d._id}`}  ><strong>{d.name}</strong></Link>
                  <div className='basis-1/4 '>
                    {d.name && (
                      
                      <img className='mx-auto grow-0 max-h-24 border-2 border-blue-600 rounded-lg' src={`/images/${d.name.replace(" M", "m")}.png`} onError={({currentTarget}) => {
