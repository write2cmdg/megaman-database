...
const BossPage = () => {
  ...
  const handleDelete = (id) => {
    const confirm = window.confirm(
      `Delete Robot Master: ${data.name} Permanently?`
    );
    if (confirm) {
      axios
    ...
    <div className=" bg-slate-800/90">
      ...
      {!data.name && (
        <div className="flex flex-col items-center p-10">
          <h2 className="py-10 flex flex-row justify-center sm:text-xl text-lg capitalize font-bold text-blue-400">
      ...
      {data && data.name && (
        <div className="py-8 flex flex-row justify-evenly lg:h-80 h-44  bg-slate-800/90 overflow-hidden overflow-y-auto scrollbar scrollbar-rounded-large scrollbar-thumb-blue-900 scrollbar-track-blue-900/40 hover:scrollbar-thumb-blue-200/50">
          ...
          {data.name && (
            <img
              className="border-8 border-blue-600 rounded-lg"
              src={`/images/${imgName.replace(" M", "m")}.png`}
              alt=""
